 ...
 */
package com.example.sensorgraph;

import com.example.sensorgraph.R;
...
/**
 * <h3>Application that receives sensor readings from Arduino displaying it
 * graphically.</h3>
 * 
 * This example demonstrates how to catch data sent from Arduino forwarded by
 * Amarino 2.0. SensorGraph registers a BroadcastReceiver to catch Intents with
 * action string: <b>AmarinoIntent.ACTION_RECEIVED</b>
 * 
...
public class SensorGraph extends Activity {
	...
	public class ArduinoReceiver extends BroadcastReceiver {
		...
		boolean hadUnregisteredFlag = false;
		...
		public void onReceive(Context context, Intent intent) {
			...
			if (dataType == AmarinoIntent.STRING_EXTRA) {
				...
				if (data != null) {
					...
					if (!data.startsWith("Flag not registered:")) {

						if (!hadUnregisteredFlag) {
							mValueTV.setText(data);
							try {
								// since we know that our string value is an int
								// number we can parse it to an integer
								final int sensorReading = Integer
										.parseInt(data);
							...
							hadUnregisteredFlag = false;
						}
					} else {
						hadUnregisteredFlag = true;
					}
				}
			}
		}
